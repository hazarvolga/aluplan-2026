    ...
    <section className="py-16 bg-transparent">
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 gap-8">
          {tables.map((t, idx) => (
            <Card key={idx} className="p-8 bg-slate-100 dark:bg-white/5 border-slate-200 dark:border-white/10 backdrop-blur-sm">
              <ul className="columns-1 md:columns-2 gap-x-12 gap-y-4">
                {t.rows.map((r, i) => (
                  <li key={i} className="mb-3 break-inside-avoid text-muted-foreground font-medium flex items-start">
