        ...
        <section className="relative min-h-[60vh] flex items-center justify-center overflow-hidden">
          ...
          <div className="container mx-auto px-4 py-20 relative z-10">
            <div className="max-w-3xl">
              <span className="text-accent font-medium text-sm tracking-wider uppercase mb-4 block">/ {badge}</span>
              <h1 className="text-4xl md:text-5xl lg:text-6xl font-bold text-foreground mb-6">
                {title}
                {subtitle && <span className="block text-2xl md:text-3xl lg:text-4xl mt-2 text-muted-foreground">{subtitle}</span>}
              </h1>
              ...
              <div className="flex flex-wrap gap-4">
                {showCtaButton && (
                  <Button size="lg" asChild className="bg-accent hover:bg-accent/90 text-accent-foreground">
                    {ctaHref ? (
                      <a href={ctaHref} target={ctaExternal ? "_blank" : undefined} rel={ctaExternal ? "noopener noreferrer" : undefined}>
                        {ctaTitle}<ArrowRight className="ml-2 h-5 w-5" />
                      </a>
                    ) : (
                      <span>{ctaTitle}</span>
                    )}
                ...
                {additionalButtons && additionalButtons.map((button, index) => (
                  <Button key={index} variant={button.variant || "outline"} size="lg" asChild>
                    <a href={button.href} target={button.external ? "_blank" : undefined} rel={button.external ? "noopener noreferrer" : undefined}>
                      {button.text}
                    </a>
                  </Button>
                ))}
