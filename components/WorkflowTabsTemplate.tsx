      ...
      <div className="container mx-auto px-4">
        ...
        <Tabs defaultValue={defaultCategory} className="w-full">
          <div className="flex justify-start mb-12">
            <TabsList className="inline-flex h-auto p-1 bg-muted/30 backdrop-blur-sm rounded-lg border border-border/50 flex-wrap justify-start gap-1">
              {categories.map((cat, i) => (
                <TabsTrigger
                  key={cat.key}
                  value={cat.key}
                  className="relative px-5 py-3 text-sm font-medium rounded-md transition-all duration-200 data-[state=active]:bg-accent data-[state=active]:text-accent-foreground data-[state=active]:shadow-sm data-[state=inactive]:hover:bg-muted/50 data-[state=inactive]:text-muted-foreground"
                >
          ...
          {categories.map((cat) => (
            <TabsContent key={cat.key} value={cat.key} className="animate-fade-in">
              <Tabs defaultValue={`${cat.key}-0`} orientation="vertical" className="flex flex-col lg:flex-row lg:items-start gap-8">
                <TabsList className="flex lg:flex-col h-auto bg-card/30 backdrop-blur-sm p-2 rounded-lg border border-border/50 lg:w-[320px] flex-shrink-0 gap-1 overflow-x-auto lg:overflow-visible">
                  {cat.items.map((item, index) => (
                    <TabsTrigger
                      key={`${cat.key}-${index}`}
                      value={`${cat.key}-${index}`}
                      className="group relative justify-start text-left w-full min-w-[200px] lg:min-w-0 py-3 px-4 rounded-md transition-all duration-200 data-[state=active]:bg-accent/10 data-[state=active]:text-accent data-[state=inactive]:hover:bg-muted/30 data-[state=inactive]:text-muted-foreground border border-transparent data-[state=active]:border-accent/20"
                    >
                      <span className="flex items-center gap-3">
                        <span className="w-6 h-6 rounded flex items-center justify-center text-[10px] font-mono border border-current opacity-60">
                          {index + 1}
                        </span>
                ...
                <div className="flex-1 min-w-0">
                  {cat.items.map((item, index) => (
                    <TabsContent key={`${cat.key}-content-${index}`} value={`${cat.key}-${index}`} className="mt-0 animate-fade-in">
                      <Card className="bg-card/50 backdrop-blur-sm border-border/50 overflow-hidden rounded-lg shadow-sm">
                        {item.image && (
                          <div
                        ...
                        <CardContent className="p-6">
                          ...
                          {item.videoUrl && (
                            <div className="mt-6">
                              <Button variant="outline" className="gap-2" onClick={() => setVideoUrl(item.videoUrl!)}>
