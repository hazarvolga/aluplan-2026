...
const description = "Allplan Bridge 2024 ile daha hızlı olun:";

const heroDescriptionNode = (
  <>
    <p className="text-lg md:text-xl lg:text-2xl text-white/70 leading-relaxed max-w-2xl font-light">Allplan Bridge 2024 ile daha hızlı olun:</p>
...
const infoBlocks: InfoBlock[] = [
  ...
  { title: "Otomatik olarak türetilen hesaplama eylemleri", subtitle: "Konsol yapım yöntemi için özel yükler", descriptions: [
    ...
    "Allplan Bridge 2024'teki yeni dengeli konsol görevi, yalnızca form gezgininin geometrik verilerinin obje odaklı girdisini ve ayrıca belirli inşaat adımlarının zaman çizelgesini gerektirir. Tüm bu veriler net bir şekilde düzenlenmiş tek bir giriş penceresinde sağlanırken, kopçanın uygulanması/hareket ettirilmesi, segmentin dökülmesi/montajı, kendi ağırlığının uygulanması, tendonların ön gerilmesi gibi çok sayıda hesaplama işleminin otomatik olarak gerçekleştirilmesiyle sonuçlanacaktır. ve değişen tüm yük uygulamaları sırasında sünme ve büzülmenin uygun şekilde dikkate alınması.",
  ...
  { title: "Allplan Bridge Raporlama Aracı", subtitle: "Raporlama sürecinde devrim yaratın", descriptions: [
    "Allplan Bridge 2024 iki dünyayı tek bir dünyada birleştiriyor ve bu sayede verimliliği, doğruluğu ve görsel etkiyi artıran bir dizi güçlü özellik sunarak raporlama sürecinde devrim yaratıyor. Tablolar, görüntüler, 2B ve 3B diyagramlar ve çok daha fazlası biçimindeki köprü tasarımı ve analiz verileri, kolayca bağlantılandırılabilir ve doğrudan MS Word belgelerine yerleştirilebilir. Bu, manuel veri girişi ve güncelleme ihtiyacını ortadan kaldırarak zamandan tasarruf sağlar ve hata riskini azaltır.",
...
const stats = [
  { value: "100%", label: "TUTARLI VERİ", description: "Tek bir yazılım çözümü ile her şey başarıldığı için - ortak veri ortamı, farklı çözümler arasında gereksiz veri alışverişini ve yeniden girişi en aza indirir." },
  { value: "63%", label: "DAHA FAZLA ZAMAN", description: "Tasarım aşaması boyunca meydana gelen proje şablonlarının ve model değişikliklerinin hızlı, kolay ve güvenilir bir şekilde uyarlanmasıyla zaman tasarrufu." },
  { value: "100%", label: "DAHA HASSAS", description: "Köprüler için özel olarak uyarlanmış modelleme teknikleri ve prefabrik kirişli köprüler gibi farklı köprü tipleri için iş akışları sayesinde." },
  { value: "100%", label: "DAHA FAZLA VERİMLİLİK", description: "Parametrik model açıklaması sonucunda verilerin yeniden kullanımı - modellenen köprü elemanları şablon olarak kaydedilebilir ve sonraki projelerde yeniden kullanılabilir." },
];

const postHeroSection = (
  <>
    <InfoMediaTwoColumn
      title="Allplan Bridge 2024 – Genel Bakış"
      description={"Yeni Allplan Bridge 2024 versiyonu parametrik köprü tasarımını tamamen yeni bir seviyeye çıkarıyor. Güçlü ve otomatikleştirilmiş modelleme ve detaylandırma araçlarını, genişletilmiş yapısal analiz yeteneklerini ve kolaylaştırılmış dijital tasarım iş akışlarını kullanarak görevleri daha önce hiç bu kadar hızlı tamamlayamamıştınız. Köprü mühendislerine tasarımlarını optimize etme, projeleri daha erken teslim etme, müşterileri için değeri en üst düzeye çıkarma ve daha iyi köprüler inşa etme fırsatı vermek."}
